...
MediaWiki.pageTitleToFilenameBase = function(title, allowSlashes) {
    ...
    var filenameBase = title
        .replace(/\\/g, '/')
        .replace(invalidFilenameChar, '_');
    if (!allowSlashes) {
    ...
    self._request(reqOptions, function(error, response, body) {
        if (error) {
            ...
            if (
                ...
                /^text\/html\b/.test(response.headers['content-type'])
            ) {
                var titleMatch = response.body.match(
                    /<h2 class="pagetitle">([^<]+)<\/h2>/
                );
                var messageMatch = response.body.match(
                    /<div id="mw-content-text"><p>([^<]+)<\/p>/
                );
            ...
            if (!reqOptions.response) {
                reqOptions.response = _.pick(response, 'headers', 'statusCode', 'body');
            }
            cb(self._error(
                reqOptions.response.title || (
